...
class COVIDxDataset(Dataset):
    ...
    def load_image(self, img_path, dim, augmentation='test'):
        ...
        # image_tensor = transform.Grayscale(image)
...
def main():
    ...
    train_features = train_data[random.randint(0,len(train_data))]
    img = train_features[0].numpy().transpose(1,2,0)
    print(img.shape)
    plt.imshow(img, cmap='gray')
    plt.show()

    # gray_img = T.Grayscale()(train_features)
    # plt([train_features[0].squeeze().numpy()], cmap='gray')
